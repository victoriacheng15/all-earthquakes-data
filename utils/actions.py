...
from utils.handle_csv import (
    setup_csv,
    get_existing_csv_codes,
    append_data_to_csv,
    get_all_csv_data,
)
...
def csv_action():
    ...
    filtered_data = tuple(
        [data for data in api_data if data["code"] not in codes_count_before]
    )
...
def db_action():
    ...
    try:
        ...
        filtered_data = tuple(
            [data for data in all_csv_data if data["code"] not in db_existing_codes]
        )
        logger.info(f" Total number of rows to be inserted: {len(filtered_data)}")
        ...
        logger.info(
            f" Total number of rows from DB after: {len(db_existing_codes_after)}"
        )
